...
ressarcimento_uc_ocorrencia AS (
    SELECT DISTINCT
        TRIM(CAST(NUM_OCORRENCIA_ADMS AS VARCHAR)) AS NUM_OCORRENCIA_ADMS,
...
ressarcimento_uc_peso AS (
    SELECT
        NUM_UC_UCI,
        COUNT(DISTINCT NUM_OCORRENCIA_ADMS) AS QTD_OCORRENCIAS_UC
    FROM ressarcimento_uc_ocorrencia
    GROUP BY NUM_UC_UCI
),
ressarcimento AS (
    SELECT
