...
WITH agregados_seq AS (
    SELECT 
        ...
        COALESCE(MODE(r.KVA_INTRP_PRIM_HIADMS), 0) AS moda_kva,
        COUNT(DISTINCT r.NUM_UC_UCI_CHVP_HIADMS) AS contagem_cons
    FROM adms_iqs_alterados t
    JOIN raw_db.hiadms_raw r
      ON CAST(r.NUM_SEQ_INTRP_CHVP_HIADMS AS VARCHAR) = t.NUM_SEQ_INTRP
     AND CAST(r.NUM_UC_UCI_CHVP_HIADMS AS VARCHAR) = t.NUM_UC_UCI
...
SELECT DISTINCT
            ...
            CAST(a.contagem_cons AS VARCHAR) AS CONS_INTRP,
            REPLACE(CAST(a.moda_kva AS VARCHAR), '.', ',') AS KVA_INTRP,
            ...
            COALESCE(
                REGEXP_REPLACE(NULLIF(TRIM(t.NUM_MOTIVO_TRAT_DIF_UCI), ''), '\.0+$', ''),
                REGEXP_REPLACE(TRIM(CAST(r.NUM_MOTIVO_TRAT_DIF_UCI_ULT_HIADMS AS VARCHAR)), '\.0+$', '')
        ...
        JOIN raw_db.hiadms_raw r
         ...
         AND CAST(r.NUM_UC_UCI_CHVP_HIADMS AS VARCHAR) = t.NUM_UC_UCI
         AND CAST(r.PID_OCOR_INTRP_ULT_HIADMS AS VARCHAR) = t.NUM_OCORRENCIA_ADMS
        LEFT JOIN agregados_seq a
          ON a.NUM_OCORRENCIA_ADMS = CAST(r.PID_OCOR_INTRP_ULT_HIADMS AS VARCHAR)
         AND a.NUM_SEQ_INTRP = CAST(r.NUM_SEQ_INTRP_CHVP_HIADMS AS VARCHAR)
        WHERE r.DATA_HORA_FIM_INTRP_ULT_HIADMS >= r.DATA_HORA_INIC_INTRP_ULT_HIADMS
    """)
...
WITH base AS (
    SELECT
        ...
        REGEXP_REPLACE(TRIM(CAST(NUM_MOTIVO_TRAT_DIF_UCI_ULT_HIADMS AS VARCHAR)), '\.0+$', '') AS NUM_MOTIVO_TRAT_DIF_UCI_ORIG,
