...
3. **API e Falhas de Renderização Frontend:** Garanta sempre que a API devolva os tipos de dados esperados pelo frontend (React). A interface gráfica da página de **Tratativas em Massa** foi migrada para ler exclusivamente as anomalias geradas pelo PostgreSQL (`/api/anomalias/amostra/{codigo}`), eliminando a dependência do DuckDB na renderização de métricas e amostras e unificando as informações.
