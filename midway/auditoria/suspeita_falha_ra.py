...
def analisar_suspeita_falha_ra(*args, **kwargs):
    load_dotenv()
    
    # Suporta receber anomes via kwargs ou primeiro argumento posicional
    anomes = kwargs.get("anomes")
    if not anomes and args:
        anomes = args[0]
    if not anomes:
    ...
    try:
        ...
        # Busca dados de ocorrências de RA
        ...
        resumo_keys = resumo[["REGIONAL", "CONJUNTO", "ALIM_INTRP", "NUM_OPER_CHV_INTRP", "DIA_OPERACAO"]].drop_duplicates()
        detalhe = ocorrencias.merge(resumo_keys, on=["REGIONAL", "CONJUNTO", "ALIM_INTRP", "NUM_OPER_CHV_INTRP", "DIA_OPERACAO"], how="inner")
